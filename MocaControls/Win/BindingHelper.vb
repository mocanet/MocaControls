...
Namespace Win
	...
	''' </summary>
	''' <remarks></remarks>
	<Obsolete("use Moca.Win.DataBinder")> _
...
#Region " プロパティ "
		...
		Public Property DataSource() As DataTable
			Get
				Return DirectCast(_bindSrc.DataSource, DataTable)
			End Get
			Set(ByVal value As DataTable)
				...
				Else
					_ds = value.DataSet
				End If
				_bindSrc.DataSource = value
				'Me.DataMember = value.TableName
		...
		Public Property DataMember() As String
			Get
				Return _bindSrc.DataMember
			End Get
			Set(ByVal value As String)
				_bindSrc.DataMember = value
			End Set
		End Property
		...
		Public Property Position() As Integer
			Get
				Return _bindSrc.CurrencyManager.Position
			End Get
			Set(ByVal value As Integer)
				_bindSrc.CurrencyManager.Position = value
			End Set
		End Property
		...
		Public ReadOnly Property CurrentRow() As DataRow
			Get
				Return DirectCast(_bindSrc.Current, DataRowView).Row
			End Get
		End Property
...
#Region " データ編集状態制御 "
		...
		Public Sub EndCurrentEdit()
			_bindSrc.CurrencyManager.EndCurrentEdit()
			Me.CurrentRow.EndEdit()
		End Sub
...
#Region " データバインディング "
		...
		''' <param name="nullValue">データ ソースに <see cref="DBNull"/> 値が格納されている場合にコントロール プロパティとして設定される <see cref="Object" /> を取得または設定します。</param>
		''' <remarks></remarks>
		Public Sub DataBinding(ByVal ctrl As TextBox, ByVal dataMember As String, Optional ByVal dataSourceNullValue As Object = Nothing, Optional ByVal nullValue As Object = Nothing)
			_dataBinding(ctrl, "Text", dataMember, dataSourceNullValue, nullValue)
			' TODO
			'txt.MaxLength = 0
		End Sub
		...
		''' </remarks>
		Private Sub _dataBinding(ByVal obj As IBindableComponent, ByVal propertyName As String, ByVal dataMember As String, ByVal dataSourceNullValue As Object, ByVal nullValue As Object)
			Dim bind As Binding
			bind = New Binding(propertyName, _bindSrc, dataMember)
		...
		Public Sub DataBinding(ByVal pbx As PictureBox, ByVal dataMember As String)
			Dim bind As Binding
			bind = New Binding("Image", _bindSrc, dataMember, True, DataSourceUpdateMode.OnPropertyChanged)
		...
		Public Sub DataBinding2(ByVal pbx As PictureBox, ByVal dataMember As String)
			Dim bind As Binding
			bind = New Binding("ImageLocation", _bindSrc, dataMember, True, DataSourceUpdateMode.OnPropertyChanged)
